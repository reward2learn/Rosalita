...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    // Build a clean column key array (non-empty headers only, in original order).
    // Empty headers get autogenerated keys that we filter out.
    const columns = headers.filter((h) => h.trim().length > 0);

    // Parse data. Using `header` as an array sets those as column keys.
    // This avoids the `__EMPTY` key issue from using header as a number.
