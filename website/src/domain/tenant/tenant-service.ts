...
export async function upsertFullTenantConfig(
  ...
  try {
    ...
    const fullConfig = {
      ...
      displayName: (additionalConfig.displayName as string) || slug,
      primaryColor: (additionalConfig.primaryColor as string) || process.env.PRIMARY_COLOR || '#eb3d28',
      secondaryColor: (additionalConfig.secondaryColor as string) || process.env.SECONDARY_COLOR || '#0af9fe',
      template,
      configVersion: '2.0',
